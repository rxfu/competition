	                        ...
	                        <td>
								...
								@can($model . '.pdf')
									@if (config('components.' . $model . '.pdf'))
										<a href="{{ route($model . '.pdf', $item->getKey()) }}" class="btn btn-success btn-flat btn-sm" title="生成推荐表">
											<i class="icon fa fa-file-pdf"></i> 生成推荐表
										</a>
										<a href="{{ route($model . '.recommend', $item->getKey()) }}" class="btn btn-warning btn-flat btn-sm" title="上传推荐表">
